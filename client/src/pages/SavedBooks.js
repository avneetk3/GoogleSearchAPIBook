...
//import more files for editing and adding
...
//}
  ...
  // const [deleteBook, {error}] = useMutation(REMOVE_BOOKS);
  ...
  //adding code for hooks and mutation 
   // useQuery hook
   const { data: userData, loading } = useQuery(GET_MYID);
   // useMutation hook
   const [remove_book, { error }] = useMutation(REMOVE_BOOKS);
 
   if (loading) {
     return <h2>LOADING...</h2>;
   }
 
   // create function that accepts the book's mongodb  _id value as parameter and delete  book from  database
   const handleDeleteBook = async (bookId) => {
     const token = Auth.loggedIn() ? Auth.getToken() : null;
